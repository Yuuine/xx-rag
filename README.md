...
- **RRF 融合**：使用 Reciprocal Rank Fusion 算法融合文本检索和向量检索结果
- **重排序服务**：集成 Python 微服务，使用 BERT-like 模型对检索结果进行重排序，提升答案准确性
...
- **架构验证**：使用 ArchUnit 确保模块间依赖关系符合整洁架构设计
...
- **框架**：FastAPI (Python)
- **模型**：sentence-transformers 交叉编码器模型
- **端口**：8082
...
│   ├── config/                      # 配置类
│   ├── exception/                   # 异常处理
│   └── aop/                        # AOP 切面（日志追踪、性能监控）
├── src/main/resources/              # 配置文件
├── src/test/java/                  # 测试源码
│   └── yuuine/xxrag/
│       └── architecture/           # 架构规则测试（ArchUnit）
├── frontend/                        # 前端项目
│   ├── src/
│   │   ├── components/              # Vue 组件
│   │   │   ├── icons/               # SVG 图标组件
...
app:
  rag:
    retrieval:
      ...
      recall-top-k: 20             # 检索时返回的 Top-K 结果数量（给 Rerank 用）
      candidate-multiplier: 10    # kNN 候选集倍数
      rrf:
        k: 60                     # 平滑常数
        text-weight: 1.0          # 文本检索（BM25）权重
        vector-weight: 1.0        # 向量检索（kNN）权重
        final-top-k: 20           # RRF 融合后返回数量（给 Rerank 用）
      rerank:
        enabled: true             # 启用 Rerank（使用 Python 微服务）
        service-url: http://localhost:8082  # Python Rerank 微服务地址
        top-k: 5                  # Rerank 后最终返回数量
  text-chunker:
...
**注意**：启用 Rerank 整体推理速度会降低，可酌情选择。

Rerank 服务依赖 BCEmbedding 模型，首次运行时会自动从 HuggingFace 下载。
...
Rerank 服务默认运行在 `http://localhost:8082`

详细说明请参阅 [rerank-service/README.md](rerank-service/README.md)
...
项目使用 GitHub Actions 实现自动化 CI/CD，包含以下工作流：

### Java CI with Maven (`java-ci.yml`)
- **触发**：push/PR 到 main、develop 分支
- **功能**：Maven 编译、单元测试、打包、安全扫描、代码覆盖率报告

### Comprehensive CI Pipeline (`comprehensive-ci.yml`)
- **触发**：push/PR 到 main、develop 分支，以及每周定时任务
- **功能**：
  - 编译和单元测试
  - 集成测试（MySQL、Elasticsearch 服务）
  - IK 分词器安装和索引创建
  - Docker 镜像构建验证

### Docker Build and Test (`docker-ci.yml`)
- **触发**：push/PR 到 main、develop 分支
- **功能**：
  - 构建 Java 应用
  - 启动 Elasticsearch 容器并安装 IK 分词器
  - 创建索引映射
  - 构建并测试 Docker 镜像

### CI with LLM Quality Evaluation (`llm-ci.yml`)
- **触发**：push/PR 到 main、dev 分支，PR 时额外执行 LLM 质量评估
- **功能**：
  - Java 单元测试（JaCoCo 覆盖率）
  - 前端单元测试和类型检查
  - Python Rerank 服务测试
  - LLM 响应质量评估（仅 PR）
  - 构建验证
...
### 代码质量保障
- **JaCoCo**：代码覆盖率报告，上传至 Codecov
- **ArchUnit**：架构规则测试，确保模块间依赖关系符合整洁架构
- **GitHub Security**：依赖安全扫描
...
**rag.vector_delete_outbox_events** - 向量删除异步事件表
```mysql
CREATE TABLE IF NOT EXISTS vector_delete_outbox_events
(
    id              BIGINT AUTO_INCREMENT PRIMARY KEY,
    file_md5        CHAR(32)        NOT NULL,
    created_at      TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
