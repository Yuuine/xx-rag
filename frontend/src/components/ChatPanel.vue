...
import type { StreamResponse, VectorSearchResult } from '../api/types'
...
const MESSAGES_PER_PAGE = 20 // 每页加载的消息数量
...
renderer.code = (arg: any) => {
  ...
  let highlightedCode = rawText
  if (lang && hljs.getLanguage(lang)) {
    try {
      highlightedCode = hljs.highlight(rawText, { language: lang }).value
    } catch {
      highlightedCode = hljs.highlightAuto(rawText).value
    }
  } else {
    highlightedCode = hljs.highlightAuto(rawText).value
  }
...
const state = reactive({
  ...
  modalPassword: '',
  isDragOver: false,
  currentReferences: [] as VectorSearchResult[],
  // 懒加载相关状态
  displayedCount: 0, // 当前显示的消息数量
  isLoadingMore: false, // 是否正在加载更多
  hasMoreMessages: false // 是否还有更多消息
})

const allMessages = ref<MessageItem[]>([]) // 所有消息
const displayedMessages = computed(() => {
  // 只返回需要显示的消息（从后往前取）
  const start = Math.max(0, allMessages.value.length - state.displayedCount)
...
function startNewStream() {
  ...
  // 更新显示数量
  state.displayedCount = allMessages.value.length
  scrollToBottom()
}
...
function saveMessages() {
  try {
    localStorage.setItem(MESSAGES_LOCALSTORAGE_KEY, JSON.stringify(allMessages.value))
  } catch {
...
function finalizeStream() {
  ...
  if (last && last.role === 'assistant') {
    ...
    last.references = state.currentReferences
  }
...
function appendAssistantMessage(content: string, duration = '0.00', references?: VectorSearchResult[]) {
  allMessages.value.push({
    role: 'assistant',
...
function loadMoreMessages() {
  ...
  window.setTimeout(() => {
    const newCount = Math.min(
      ...
      state.displayedCount + MESSAGES_PER_PAGE
    )
    state.hasMoreMessages = newCount < allMessages.value.length
    state.displayedCount = newCount
    state.isLoadingMore = false
  }, 300)
}

// 滚动事件处理：检测是否滚动到顶部以加载更多
function handleScroll() {
  if (!messagesContainerRef.value) return
  
  const { scrollTop } = messagesContainerRef.value
  // 当滚动到顶部附近时加载更多
  if (scrollTop < 50 && state.hasMoreMessages && !state.isLoadingMore) {
    loadMoreMessages()
  }
}
    ...
    try {
      ...
      if (data.type === 'history' && data.messages) {
        ...
        if (savedMessages.length > 0) {
          // 懒加载：只保存全部消息，但只显示最新的 MESSAGES_PER_PAGE 条
          ...
          // 将服务器消息保存到 localStorage
          saveMessages()
        }
        // 初始化懒加载状态 - 只显示最新的 MESSAGES_PER_PAGE 条
        const totalMessages = allMessages.value.length
        state.hasMoreMessages = totalMessages > MESSAGES_PER_PAGE
        state.displayedCount = Math.min(totalMessages, MESSAGES_PER_PAGE)
        scrollToBottom()
        return
      }
      ...
      if (data.finishReason === 'stop') {
        if (data.references) {
          state.currentReferences = data.references
        }
        finalizeStream()
        return
      }
...
function getUniqueReferences(references: VectorSearchResult[]): string[] {
  const unique: string[] = []
  const seen = new Set<string>()
  for (const ref of references) {
    if (ref.source && !seen.has(ref.source)) {
      seen.add(ref.source)
      unique.push(ref.source)
    }
  }
  return unique
}
  ...
  <div class="chat-card">
    ...
    <div class="chat-container">
      <div id="messagesContainer" ref="messagesContainerRef">
        <!-- 加载更多提示 -->
        <div v-if="state.isLoadingMore" class="loading-more">
        ...
        <div class="messages-list">
          <div v-for="(item, idx) in displayedMessages" :key="idx" class="message" :class="item.role">
            ...
            <!-- 引用来源显示 -->
            <div v-if="item.role === 'assistant' && item.references && item.references.length > 0" class="references">
              <strong>参考来源：</strong>
              <ol>
                <li v-for="(ref, refIdx) in getUniqueReferences(item.references)" :key="refIdx">{{ ref }}</li>
              </ol>
            </div>
          </div>
        </div>
      ...
      <!-- 快捷跳转按钮 -->
      <div v-if="allMessages.length > 1" class="quick-nav">
        <button class="nav-btn" @click="scrollToBottomSmooth" title="滚动到底部">
